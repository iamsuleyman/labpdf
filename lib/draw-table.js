...
export function drawResultTable(doc, currentY, rows, patient, doctor, specimen) {
  const tableBody = [];
  const tableRowRefs = [];
  ...
  rows.forEach(r => {
    ...
    if (r.note) {
      tableBody.push([
        { content: 'Примечание:', styles: { fontStyle: 'normal', fontSize: 5.4, textColor: S.colors.black } },
        { content: r.note, colSpan: 4, styles: { fontSize: 5.1, textColor: S.colors.black, cellPadding: { top: 1.1, bottom: 1.5, left: 2, right: 2 } } },
      ]);
      tableRowRefs.push({ type: 'note', row: r });
    }
  });

  const hooks = createTableHooks(doc, tableBody, tableRowRefs, patient, doctor, specimen);
  ...
  doc.autoTable({
    ...
    styles: {
      font: 'SourceSans3',
      fontSize: 6.5,
      cellPadding: { top: 0.55, bottom: 0.55, left: 1.8, right: 1.8 },
    ...
    columnStyles: {
      ...
      3: { cellWidth: 31, halign: 'center', cellPadding: { top: 0.55, bottom: 0.55, left: 9, right: 1.8 } },
