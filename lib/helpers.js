...
export function isOutOfRange(item, sex) {
	...
	const lo = parseFloat(
		normalizedSex === "female" ? rf.lowerBoundFemale : rf.lowerBoundMale,
	);
	const hi = parseFloat(
		normalizedSex === "female" ? rf.upperBoundFemale : rf.upperBoundMale,
	);
...
export function refInterval(rf, sex) {
	...
	if (normalizedSex === "female") {
		const otherFemale = normalizeText(rf.otherFemale);
		if (otherFemale && otherFemale !== "-") return otherFemale;
		const lo = normalizeText(rf.lowerBoundFemale);
		const hi = normalizeText(rf.upperBoundFemale);
		if (lo && lo !== "-" && hi && hi !== "-") return lo + "-" + hi;
	}
	const otherMale = normalizeText(rf.otherMale);
	if (otherMale && otherMale !== "-") return otherMale;
	const lo = normalizeText(rf.lowerBoundMale);
	const hi = normalizeText(rf.upperBoundMale);
	if (lo && lo !== "-" && hi && hi !== "-") return lo + "-" + hi;
	return "";
}
...
export function buildRowNote(item, patientSex) {
	...
	const alternateRange =
		normalizeSex(patientSex) === "female" ? rf.otherFemale : rf.otherMale;
