...
async function generatePDF(data, options = {}) {
  ...
  for (const [sampleId, groupReports] of sampleGroups) {
    ...
    doc.autoTable({
      startY: currentY,
      head: [['Показатель', 'Текущий результат', 'Ед. изм.', 'Референсный интервал']],
      body: tableBody,
      theme: 'plain',
      styles: {
        font: 'PTSans',
      ...
      columnStyles: {
        ...
        3: { cellWidth: contentW - 62 - 40 - 22, halign: 'center' },
      },
      ...
      didParseCell: function (hookData) {
        if (hookData.section === 'head') {
          ...
          if (hookData.column.index === 2 || hookData.column.index === 3) {
      ...
      willDrawCell: function (hookData) {
        if (hookData.section === 'body') {
          ...
          if (hookData.column.index === 1) {
            // draw blue fill manually, shrunk by 0.35mm top & bottom
            const inset = 0.1;
            doc.setFillColor(...S.colors.resultBg);
            doc.rect(x, y + inset, width, height - inset * 2, 'F');
      ...
      didDrawCell: function (hookData) {
        ...
        // Draw flag text at fixed position in column 1
        if (hookData.column.index === 1 && rowRef.row.flag) {
          const flagX = hookData.cell.x + 20;
          const flagY = hookData.cell.y + hookData.cell.height / 2 + 0.1;
          doc.setFont(S.font, 'bold');
          doc.setFontSize(5.9);
          doc.setTextColor(...S.colors.black);
          doc.text(rowRef.row.flag, flagX, flagY, { baseline: 'middle' });
        }
