...
function drawHeader(doc, logoPng) {
  ...
  doc.setFont(S.font, 'bold');
  doc.text(PATIENT.accountNumber, tx + doc.getTextWidth('Номер аккаунта: '), y0 + 7.8);

  doc.setFont(S.font, 'normal');
  doc.text('Назначивший врач: ', tx, y0 + 11.1);
  doc.setFont(S.font, 'bold');
  doc.text(PATIENT.orderingPhysician, tx + doc.getTextWidth('Назначивший врач: '), y0 + 11.1);
...
async function generatePDF(data, options = {}) {
  ...
  for (const [sampleId, groupReports] of sampleGroups) {
    ...
    doc.autoTable({
      startY: currentY,
      head: [['Показатель', 'Результат', 'Флаг', 'Единица измерения', 'Референсный интервал']],
      body: tableBody,
      theme: 'plain',
      styles: {
        font: 'SourceSans3',
      ...
      columnStyles: {
        ...
        3: { cellWidth: 31, halign: 'center', cellPadding: { top: 1.15, bottom: 1.15, left: 9, right: 1.8 } },
