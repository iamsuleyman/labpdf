...
function loadLogoPng(svgUrl) {
  if (_logoPng) return Promise.resolve(_logoPng);
...
function resolvePatient(options) {
  const patient = {
    ...DEFAULT_PATIENT,
    ...((options && (options.patient || options.user)) || {}),
  };

  if (!patient.name) {
    throw new Error('LabPdf.generatePDF requires options.patient (or options.user) with at least a name.');
  }
...
function drawHeader(doc, logoPng) {
  ...
  doc.text('ID образца: ' + PATIENT.specimenId, S.margin.l, y0 + 11.1);
  ...
  doc.setFontSize(8.5);
  doc.setTextColor(...S.colors.black);
  doc.text(PATIENT.dob, cx + 18, y0 + 3.8);

  doc.setFont(S.font, 'normal');
  doc.setFontSize(5.6);
  doc.setTextColor(...S.colors.muted);
  doc.text('Возраст: ' + PATIENT.age, cx, y0 + 7.7);
  doc.text('Пол: ' + displaySex(PATIENT.sex), cx, y0 + 11);
  ...
  doc.setFontSize(5.6);
  doc.setTextColor(...S.colors.muted);
  doc.text('Номер аккаунта: ' + PATIENT.accountNumber, tx, y0 + 7.8);
  doc.text('Назначивший врач: ' + PATIENT.orderingPhysician, tx, y0 + 11.1);
...
function drawDateBar(doc, y) {
  ...
  doc.setFontSize(5.9);
  const items = [
    ['Дата взятия: ', PATIENT.dateCollected],
    ['Дата получения: ', PATIENT.dateReceived],
    ['Дата отчета: ', PATIENT.dateReported],
    ['Натощак: ', displayFasting(PATIENT.fasting)],
  ];
...
function drawFooter(doc, pageNum, totalPages, logoPng) {
  ...
  if (logoPng) {
    ...
    doc.addImage(logoPng, 'PNG', S.margin.l, y + 1, fLogoW, fLogoH);
  }

  // center text
  doc.setFont(S.font, 'normal');
  doc.setFontSize(4.9);
  const center = 'Дата создания и сохранения  06/26/24 0612 ET';
  doc.text(center, S.page.w / 2 - 6, y + 4.8);
  doc.setFont(S.font, 'bold');
  doc.text('Финальный отчет', S.page.w / 2 + 26, y + 4.8);
  doc.setFont(S.font, 'normal');
  ...
  // bottom right
  doc.setFontSize(4.1);
  const rText1 = 'Этот документ содержит частную и конфиденциальную медицинскую информацию, защищенную законом.';
  const rText2 = 'Если вы получили этот документ по ошибке, позвоните по номеру 800-631-5250';
...
function drawDetailsSection(doc, y, logoPng) {
  ...
  doc.setFont(S.font, 'normal');
  doc.setFontSize(5.1);
  const disclaimer = 'Предыдущий результат указывается для самого недавнего теста, выполненного Labcorp за последние 5 лет, если демографических данных пациента достаточно для сопоставления результата с пациентом. Результаты некоторых тестов не отображаются в разделе предыдущих результатов.';
  ...
  doc.setTextColor(...S.colors.black);
  doc.text('Вне референсного диапазона', S.margin.l + 2.5, y);
  ...
  doc.setFont(S.font, 'bold');
  doc.setFontSize(6.6);
  doc.text('Выполняющие лаборатории', S.margin.l, y);
  y += 3.7;
  doc.setFont(S.font, 'normal');
  doc.setFontSize(5.1);
  doc.text('01: RN - Labcorp Raritan, 69 First Avenue, Raritan, NJ 08869-1800 Директор: Liza Jodry, MD', S.margin.l, y);
  y += 2.5;
  doc.text('По вопросам врач может связаться с отделением: 800-631-5250, лаборатория: 800-631-5250', S.margin.l, y);
  y += 6.2;
  ...
  doc.setFont(S.font, 'bold');
  doc.text('Контрольный ID: ' + PATIENT.controlId, col3, y);
  y += 2.8;
  doc.setFont(S.font, 'normal');
  ...
  doc.text('Телефон: ' + PATIENT.physicianPhone, col2, y);
  doc.text('Дата взятия: ' + PATIENT.dateCollected + ' Local', col3, y);
  y += 2.8;
  doc.text('Возраст: ' + PATIENT.age, col1, y);
  doc.text('Номер аккаунта: ' + PATIENT.accountNumber, col2, y);
  doc.text('Дата получения: ' + PATIENT.dateReceived + ' 0000 ET', col3, y);
  y += 2.8;
  doc.text('Пол: ' + displaySex(PATIENT.sex), col1, y);
  doc.text('NPI: ' + PATIENT.physicianNPI, col2, y);
  doc.text('Дата отчета: ' + PATIENT.dateReported + ' 0607 ET', col3, y);
}

// ─── Main: generatePDF ──────────────────────────────────────────────
async function generatePDF(data, options = {}) {
  PATIENT = resolvePatient(options);
  ...
  const logoPng = await loadLogoPng(logoUrl);
  ...
  for (const [sampleId, groupReports] of sampleGroups) {
    // Sample group header
    if (sampleGroups.size > 1 && sampleId) {
      ...
      doc.setFontSize(6.5);
      doc.setTextColor(...S.colors.muted);
      doc.text('ID образца: ' + sampleId, S.margin.l, currentY + 3.5);
      doc.setDrawColor(...S.colors.rowBorder);
      doc.setLineWidth(0.13);
      doc.line(S.margin.l, currentY + 5, S.page.w - S.margin.r, currentY + 5);
      currentY += 7;
    }
    ...
    doc.autoTable({
      startY: currentY,
      head: [['Показатель', 'Текущий результат', 'Единица измерения', 'Референсный интервал']],
      body: tableBody,
      theme: 'plain',
      styles: {
        font: 'SourceSans3',
      ...
      columnStyles: {
        0: { cellWidth: 58 },
        1: { cellWidth: 38 },
        2: { cellWidth: 31, halign: 'center', cellPadding: { top: 1.15, bottom: 1.15, left: 9, right: 1.8 } },
        3: { cellWidth: contentW - 58 - 38 - 31, halign: 'center' },
      },
      ...
      didParseCell: function (hookData) {
        if (hookData.section === 'head') {
          ...
          if (hookData.column.index === 0) {
            hookData.cell.styles.cellPadding = { top: 1.1, bottom: 1.3, left: 7, right: 1.8 };
          }
          if (hookData.column.index === 1 || hookData.column.index === 2 || hookData.column.index === 3) {
            hookData.cell.styles.halign = 'center';
          }
          if (hookData.column.index === 2) {
            hookData.cell.styles.cellPadding = { top: 1.1, bottom: 1.3, left: 9, right: 1.8 };
          }
        }
        ...
        if (hookData.column.index === 1) {
          ...
          hookData.cell.styles.cellPadding = { top: 1.15, bottom: 1.15, left: 10, right: 10 };
        }
      ...
      willDrawCell: function (hookData) {
        if (hookData.section === 'body') {
          ...
          if (hookData.column.index === 1) {
            // draw blue fill manually; first row gets extra 1mm top inset
      ...
      didDrawCell: function (hookData) {
        ...
        // Draw flag text right-aligned with 10mm padding from right edge
        if (hookData.column.index === 1 && rowRef.row.flag) {
          const flagX = hookData.cell.x + hookData.cell.width - 10;
          const flagY = hookData.cell.y + hookData.cell.height / 2 + 0.1;
          doc.setFont(S.font, 'bold');
          doc.setFontSize(5.9);
          doc.setTextColor(...S.colors.black);
          doc.text(rowRef.row.flag, flagX, flagY, { baseline: 'middle', align: 'right' });
        }
  ...
  for (let p = 1; p <= totalPages; p++) {
    doc.setPage(p);
    drawFooter(doc, p, totalPages, logoPng);
  }
...
function createLabReportPdf(data, options = {}) {
...
if (typeof window !== 'undefined') {
  window.LabPdf = LabPdf;
  window.generatePDF = generatePDF;
}

if (typeof module !== 'undefined' && module.exports) {
  module.exports = LabPdf;
}
